...
device_num = "/gpu:0"
model_name = "2-layer_RegularizedRNN-0.001-l1_5000_epochs"
...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        rnn1_weights = tf.get_default_graph().get_tensor_by_name('RNN1/basic_rnn_cell/kernel:0')
        rnn2_weights = tf.get_default_graph().get_tensor_by_name('RNN2/basic_rnn_cell/kernel:0')
        rnn1_eigenvalues, _, _ = tf.svd(rnn1_weights, name='rnn1_eigenvalues')
        rnn2_eigenvalues, _, _ = tf.svd(rnn2_weights, name='rnn2_eigenvalues')
        regularizer = tf.contrib.layers.l1_regularizer(scale=L1_Regularization_Weight)
        tf.contrib.layers.apply_regularization(regularizer, 
                [rnn1_eigenvalues - tf.ones_like(rnn1_eigenvalues), 
                    rnn2_eigenvalues - tf.ones_like(rnn2_eigenvalues)])
        print('get trainable variables from model: {}'.format(tf.trainable_variables()))
        print('get rnn1 weights: {}\n shape: {}\n eigenvalues: {}'.format(rnn1_weights, tf.shape(rnn1_weights), rnn1_eigenvalues))
        print('get rnn2 weights: {}\n shape: {}\n eigenvalues: {}'.format(rnn2_weights, tf.shape(rnn2_weights), rnn2_eigenvalues))
...
dim_image = 4096
dim_hidden= 1000
L1_Regularization_Weight = 0.001  # originally 0.01
